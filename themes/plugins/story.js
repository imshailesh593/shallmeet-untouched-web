...
(function () {
  ...
  this.Story = function (data) {
    ...
    var baseHTML =
      ...
      "</div>" +
      '<div id="chat-input-send" data-story-chat-input><i onclick="sendMessageFromStory(' +
      storyUser +
      ')" class="icon icon--lg icon--white ion-android-send"></i>' +
      "</div>" +
      ...
      '<div class="paused"><span>' +
      site_lang[614]["text"] +
      "</span></div>" +
      '<div class="story-icon vivify popIn box-shadow" onclick="socialStory.close(); goToProfile(' +
      storyUser +
      ');"></div> <span class="story-text text-shadow" onclick="socialStory.close(); goToProfile(' +
      storyUser +
      ');" style="cursor:pointer;"></span><span class="story-date text-shadow"></span></div>' +
      '<div class="story-nav-right">' +
      '<div class="storySettings"><i class="icon ion-android-more-horizontal"></i></div>' +
      '<div class="storyCredit" data-my-story="' +
      myStory +
      '"><img class="top-menu-credits" src="' +
      site_config.theme_url +
      '/images/coin.png" style="width: 30px;position: absolute;left: 3px;top: 0px"><span data-story-credits></span></div>' +
    ...
    icon.setAttribute(
      "style",
      "width:50px;height:50px;background-image:url(" +
        defaults.playlist[0].icon +
        ")"
    );
    ...
    if (plugins["story"]["storyCredits"] == "Yes") {
      $(".storyCredit").click(function () {
        if (myStory == 1) {
          ...
          storyCreditsArray = JSON.parse(
            "[" + plugins["story"]["storyCreditsValues"] + "]"
          );
          storyCreditsArray.forEach(function (element) {
            creditsBtn[element + " " + site_lang[285]["text"]] =
              start + "," + storyId + "," + element + ",Credits";
          });
  ...
  function launch() {
    ...
    if (defaults.playlist[start].stype == "video") {
      storyContainer.innerHTML =
        '<video class="story-next videoStory" src="" playsinline></video><div class="leftSection" onclick="socialStory.prev();"></div><div class="rightSection" onclick="socialStory.next();"></div>';
      ...
      video.oncanplay = function () {
        ...
        video.play();
        document
          .getElementsByClassName("story-video")[0]
          .setAttribute("style", "min-width: " + video.offsetWidth + "px;");
        video.muted = false;
      };
      ...
      thisTimeline.style.width = "0%";
      storyContainer.innerHTML =
        '<img class="story-next videoStory" src="" /><div class="leftSection" onclick="socialStory.prev();"></div><div class="rightSection" onclick="socialStory.next();"></div>';
    ...
    if (
      ...
      $(".story-window").append(
        '<div class="story-blur gradient8 box-shadow" onclick="socialStory.purchaseStory(' +
          storyId +
          "," +
          storyCredits +
          ')"><span>' +
    ...
    if (storyReview == "Yes" && plugins["story"]["showStoryInReview"] == "No") {
      ...
      $(".story-window").append(
        '<div class="story-blur box-shadow" style="background:#fff"><span style="color:#222">' +
          site_lang[697]["text"] +
          "</span></div>"
      );
      $("[data-story-chat-input]").hide();
  ...
  function purchaseStory(sid, credits) {
    ...
    data.message =
      site_lang[610].text + " " + credits + " " + site_lang[73].text;
    if (mobileSite) {
    ...
    if (start >= defaults.playlist.length) {
      setTimeout(function () {
        if (storyPage == "discover" && viewingStory < totalDiscoverStories) {
          ...
          if (mobileSite) {
            angular
              .element(document.querySelector("#story" + viewingStory))
...
$("#upload-story").dmUploader({
  ...
  onNewFile: function (id, file) {
    ...
    uploadingStoryFakeId = Math.floor(
      Math.random() * (90000 - 10000 + 1) + 10000
    );
    newStoryUpload = true;
    newStory["credits"] = 0;
  ...
  onUploadProgress: function (id, percent) {
    var uploadingStoryTimeline =
      document.getElementsByClassName("story-active-0")[0];
    ...
    if (plugins["story"]["sendMessage"] > 0) {
      ...
      if (plugins["story"]["storyCreditsMessageTransfer"] == "Yes") {
        updateCredits(
          ...
          plugins["story"]["sendMessage"],
          2,
          "Credits for message from story"
        );
      }
    } else {
