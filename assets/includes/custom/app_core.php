...
function pushNotification($sender_id, $receiver_id, $r_id, $title, $message, $photo, $type, $is_shared = 0)
{
  $content = array(
    "en" => $message
  );
  ...
  $fields = array(
    ...
    'is_shared' => $is_shared,
    'app_id' => "f02f0ebc-fad1-49cb-893f-967fac8495ce",
...
function isSuperFanApp($uid1, $uid2)
{
  global $mysqli;
  $result = 0;
  $query = $mysqli->query("SELECT count(*) as total FROM users_likes where u1 = '" . $uid1 . "' and u2 = '" . $uid2 . "' and love = 1 and superlike = 1");
  $total = $query->fetch_assoc();
  if ($total['total'] >= 1) {
    $result = 1;
  }
