...
function getUserStories($name, $photo, $filter = '', $order = '', $limit = '')
{
	...
	if ($query->num_rows >= 1) {
		while ($row = $query->fetch_assoc()) {
			...
			$result[$i]['bg_color'] = $row['bg_color'];
			//! TODO : Add To Database
			$result[$i]['soundId'] = $row['soundId'];
			$result[$i]['sound'] = $row['sound'];
			$result[$i]['soundType'] = $row['soundType'];
			$result[$i]['soundDuration'] = $row['soundDuration'];
			$result[$i]['soundCover'] = $row['soundCover'];
			$result[$i]['soundName'] = $row['soundName'];
			$result[$i]['postId'] = $row['id'];
			$result[$i]['postType'] = $row['postType'];
			$result[$i]['comments'] = $row['comments'];
			$result[$i]['reactions'] = $row['reactions'];
			$result[$i]['shares'] = $row['shares'];
			$result[$i]['views'] = $row['views'];
			$result[$i]['isSponsored'] = $row['isSponsored'];
			$result[$i]['sponsoredType'] = $row['sponsoredType'];
			$result[$i]['custom_sponsor'] = $row['custom_sponsor'];
			$result[$i]['sponsor_name'] = $row['sponsor_name'];
			$result[$i]['sponsor_link'] = $row['sponsor_link'];
			$result[$i]['sponsor_image'] = $row['sponsor_image'];
			$result[$i]['sponsor_text'] = $row['sponsor_text'];
...
function getLandingPage($page_url = 'index', $customLanding = '')
{
	global $sm, $lang;
	if ($customLanding != '') {
		$page = './themes/' . $customLanding . '/layout/' . $page_url . '.html';
	} else {
...
function getAdminUsersReported($limit = 0, $order = '')
{
	global $mysqli, $sm;
	$return = '';
...
function searchFriends($uid, $filter)
{
	...
	if ($query->num_rows > 0) {
		while ($result = $query->fetch_object()) {
			...
			if (isFan($uid, $sm['search']['id']) == 0) {
				$search .= '<div class="sidebar-friends" data-name="' . $sm['search']['name'] . '" data-uid="' . $sm['search']['id'] . '" 
				data-twitter="' . isFan($uid, $sm['search']['id']) . '" data-friend="' . isFan($sm['search']['id'], $uid) . '" data-conv="' . checkConv($uid, $sm['search']['id']) . '" data-message="' . getNewMessages($uid, $sm['search']['id']) . '"  data-status="' . userFilterStatus($sm['search']['id']) . '">
...
function time_elapsed_string($ptime)
{
	...
	$a_plural = array(
		$sm['lang'][307]['text'] => $sm['lang'][313]['text'],
